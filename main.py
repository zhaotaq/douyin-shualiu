...
def batch_submit():
    ...
    # 获取所有节点信息（只需一次）
    # Note: mihomo will be restarted for each node attempt within the loop
    ...
    for i, url in enumerate(urls, 1):
        ...
        while attempt_count < max_attempts and available_nodes_for_url:
            ...
            try:
                ...
                print(f'🚀 启动mihomo代理内核 for {url} with {node}...') # Added detailed log
                mihomo_proc = start_mihomo(mihomo_path, config_path)
                print(f'⏳ 等待代理API端口启动 for {url} with {node}...') # Added detailed log
                if not wait_mihomo_api(api_port):
                ...
                print(f'✅ mihomo已启动，API可用 for {url} with {node}') # Added detailed log
                
                try:
                    # Switch node - Use the selected node
                    print(f'🔄 切换节点 to {node} for {url}...') # Added detailed log
                    switch_random_node_main_group(api_port, group_name, [node])
                    print(f'✅ 节点 {node} 切换成功 for {url}') # Added detailed log
                except Exception as e:
